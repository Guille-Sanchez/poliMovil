...
export const Login = ({ setIsAuthenticated }: Props): JSX.Element => {
  ...
  return (
    <section className="bg-white pr-5 pl-5 w-full h-full pt-5">
      <form className='p-5 grid gap-6 w-full' onSubmit={e => {
        console.log('form')
        handleOnSubmit(e)
      }}>
        <div className="flex flex-col gap-2 justify-between items-center h-min">
          ...
          <input type="text" className="border border-gray-500 rounded-lg pl-5 w-full" placeholder="poligata@fpuna.edu.py" id="email" name="email"/>
        </div>
      ...
      <div className="flex flex-col gap-2 justify-between items-center h-min">
        ...
        <button className='border border-blue-900 text-blue-900 pt-2 pb-2 p-7 pr-7 rounded-lg'>Crear cuenta</button>
      </div>
    </section>
  )
}
